...
for i in range(N_balls):
    ...
    ball_object.set_environment_params(g_acceleration=g_acceleration,slow_rate=slow_rate,coefficient_of_restitution=0.7+0.2*random.random())
    ball_object.set_velocity_params2(velocity=50, angle=0+360*random.random())
    ball_object.set_bounding_limits(top=0,bottom=SCREEN_HEIGHT,left=0,right=SCREEN_WIDTH)
    all_balls.append(ball_object)
